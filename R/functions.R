...
separable_GP_param_est <- function(output_mat){
  ...
  m_eigen=try(stats::optim(param_ini,method ="L-BFGS-B",#lower = c(-Inf,-Inf,-Inf),
                    Neg_log_lik_eigen_with_nugget),silent=T) ##it will be more accurate to use derivative
  while(!is.numeric(m_eigen[[1]])){
    m_eigen=try(stats::optim(param_ini+stats::runif(3),method ="L-BFGS-B",#lower = c(-Inf,-Inf,-Inf),
                      Neg_log_lik_eigen_with_nugget),silent=T) ##it will be more accurate to use derivative
...
#' 
#' @export
predict_separable_GP <- function(output_mat, parameters){
  n1=dim(output_mat)[1]
  n2=dim(output_mat)[2]
  N=n1*n2
  p=2 ##2D input
...
criterion_1 <- function(predmean_mat, delta = 0.01, nugget = T) {
  ...
  th<- 0.05*stats::sd(diff_pixel_counts)
...
#' @param delta Step size for percentages to be tested for criterion 1 (default is 0.01)
#' @param nugget Nugget boolean for rgasp()
...
#' @param compress_output Determines if binary and cell mask results should be represented in matrix (FALSE) or list (TRUE) form; default is FALSE
#' @param return_gradient Returns vertical, horizontal, and magnitude gradients for predictive mean if TRUE; default is FALSE
#' @return Returns a list containing:
#'   \item{ori_images}{Original version of sectioned image}
#'   \item{processed_images}{Predictive mean for each image section}
#'   \item{gradients}{Vertical, horizontal, and magnitude gradients for predictive mean if `return_gradient` is TRUE}
...
#' @examples
#' # Example 1: Segmentation of JPEG nuclear channel image
#' img_path <- system.file("extdata", "example_cells.jpg", package = "FastSegmentation")
#' gp_masks_result <- generate_GP_Masks(img_path)
#' 
#' # Example 2: Segmentation of TIF whole cell channel image
#' img_path <- system.file("extdata", "whole_cell_example.tif", package = "FastSegmentation")
#' gp_masks_result <- generate_GP_Masks(img_path)
#' 
#' @export
generate_GP_Masks <- function(file_path, delta = 0.01,
                              nugget = T, middle_threshold = 0.15, boundary_threshold = 0.05,
                              compress_output = F, return_gradient = F) {
  ...
  processed_vert_grad <- list()
  processed_horz_grad <- list()
  processed_mag_grad <- list()
  ...
  for (i in 1:num_pieces_x) {
    for (j in 1:num_pieces_y) {
      ...
      separable_GP_info <- predict_separable_GP(img_matrix, parameters$param)
      predmean_mat <- separable_GP_info$predmean_mat
      predmean_vert <- separable_GP_info$grad2
      predmean_horz <- separable_GP_info$grad1
      predmean_mag <- separable_GP_info$grad_magnitude

      # Store processed images and original images
      processed_images[[count]] <- predmean_mat
      processed_vert_grad[[count]] <- predmean_vert
      processed_horz_grad[[count]] <- predmean_horz
      processed_mag_grad[[count]] <- predmean_mag
  ...
  sd_connected_parts <- stats::sd(unlist(connected_parts_count))
  outlier_threshold <- 2
  ...
  combined_vert <- matrix(0, nrow = img_height, ncol = img_width)
  combined_horz <- matrix(0, nrow = img_height, ncol = img_width)
  combined_mag <- matrix(0, nrow = img_height, ncol = img_width)
  ...
  for (i in 1:num_pieces_x) {
    for (j in 1:num_pieces_y) {
      ...
      predmean_vert <- processed_vert_grad[[count]]
      predmean_horz <- processed_horz_grad[[count]]
      predmean_mag <- processed_mag_grad[[count]]
      ...
      combined_vert[y_offset + 1:piece_height, x_offset + 1:piece_width] <- predmean_vert
      combined_horz[y_offset + 1:piece_height, x_offset + 1:piece_width] <- predmean_horz
      combined_mag[y_offset + 1:piece_height, x_offset + 1:piece_width] <- predmean_mag
  ...
  dist_map <- distmap(EBImage::as.Image(combined_thresholded1))
  ...
  if (compress_output == TRUE) {
    GP_masks <- matrix_which(GP_masks)
    combined_thresholded1 <- matrix_which(combined_thresholded1)
  }
  
  gradient_list <- NULL
  if (return_gradient == TRUE) {
    gradient_list <- list(vertical = combined_vert,
                          horizontal = combined_horz, 
...
#' 
#' This function compresses matrix outputs into a data frame containing coordinates for each value.
#' This is used for compressing the binary and cell mask outputs in the generate_GP_Masks function.
#' @param mat Numeric matrix
#' @return Data frame containing each 2D coordinate and corresponding value
#' @export
matrix_which <- function(mat) {
  df <- as.data.frame(which(!is.na(mat), arr.ind = TRUE))
  df$value <- mat[cbind(df$row, df$col)]
  return(df)
}
